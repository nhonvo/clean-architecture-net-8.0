...
public class LoggingMiddleware(RequestDelegate next, ILogger<LoggingMiddleware> logger)
{
    ...
    private async Task<bool> ShouldLogRequest(HttpContext context)
    {
        ...
        // Read request body and check its length
        using var memStream = new MemoryStream();
        await context.Request.Body.CopyToAsync(memStream);
        memStream.Seek(0, SeekOrigin.Begin);
        context.Request.Body.Position = 0; // Reset stream position for further middleware
        ...
        try
        {
            ...
            else
            {
                LogHelper.LogResponse(
                    _logger,
                    "Middleware Log Response",
                    $"Response body too large to log. ContentLength: {responseBodyText.Length} bytes",
                    context.Response.StatusCode
                );
            }
